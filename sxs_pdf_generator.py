...
# ============================================================================
# GOOGLE APPS SCRIPT INTEGRATION (Backend Only)
# ============================================================================
    ...
    def validate_question_id(self, question_id: str) -> dict:
        ...
        try:
            if not self.webhook_url:
                return {"success": True, "message": "Question ID accepted"}
...
# ============================================================================
# INTEGRATION FUNCTIONS (Clean Backend Calls)
# ============================================================================
...
def parse_question_id(question_id: str) -> tuple:
    ...
    try:
        ...
        # - human_eval_zh-CN+INTERNAL (lowercase-uppercase)
        # - human_eval_es-419+INTERNAL (lowercase-numbers)
        # - human_eval_es-en+INTERNAL (lowercase-lowercase)
...
def validate_question_id_against_sot(question_id: str) -> tuple:
    ...
    try:
        ...
        if validation_result.get("success"):
            is_valid = validation_result.get("data", {}).get("is_valid", True)
            message = "Question ID validated successfully" if is_valid else "Question ID not found"
...
def main():
    ...
    if page == "Metadata Input":
            ...
            if submitted:
                if question_id and prompt_text and model_combo:
                    ...
                    if is_valid:
                        ...
                        with col2:
                            if model_comparison:
                                st.success(f"🤖 **Model Pairing:** {model_comparison}")
                            else:
                        ...
                        <div class="error-message">
                            <strong>❌ Validation Failed:</strong> {error_details}
                        </div>
                        """, unsafe_allow_html=True)
                else:
                    st.markdown("""
        ...
        with tab2:
            ...
            #### Common Issues:
            - **Email validation fails**: Use proper email format
            - **PDF generation fails**: Check image formats and sizes
        ...
        with tab3:
            ...
            **Auto-populated from SOT:**
            ...
            |--------|--------|--------------|
            | A | Task ID | coach_P128631_quality_sxs_e2e_... |
            | B | Language | en-US |
            | C | Model Comparison | Bard 2.5 Pro vs. AIS 2.5 Pro |
            | D | Project Type | Text |
            
            #### Validation Process:
            1. **Extract Task ID** from Question ID using pattern: `bard_data+{TASK_ID}+INTERNAL`
            2. **Lookup Task ID** in SOT Tab Column A
            3. **Auto-populate** Language, Model Comparison, and Project Type from SOT
            4. **Block saving** if Task ID not found in SOT
            ...
            4. Question ID
            5. Language (from SOT)
            6. Project Type (from SOT)
            7. Prompt Text
            8. Prompt Image? (Yes/No)
            9. Model 1 (from SOT or selection)
            10. Model 1 Images (count)
            11. Model 2 (from SOT or selection)
            12. Model 2 Images (count)
            13. PDF Filename
            14. File Size (KB)
            """)
            ...
            if submitted:
                if question_id and prompt_text and model_combo:
                    ...
                    if is_valid:
                        ...
                        st.markdown("""
                        <div class="success-message">
                            <strong>✅ Success!</strong> Metadata saved successfully!
                        </div>
                        """, unsafe_allow_html=True)
                        st.balloons()
                    else:
